...
def gestion_usuarios_view(request):
    val = serial.ValidarUsuarioSerializer(data={
        "id": request.GET.get("id"),
        "nombre": (request.GET.get("nombre") or "").strip(),
        "saldo": (request.GET.get("saldo") or "").strip(),
    ...
    ctx = {
        ...
        "filtros": {
            ...
            "saldo": data.get("saldo") or "",
        },
        "csrf_token": get_token(request),
    }
    return render(request, "usuarios/GestorUsuarios.html", ctx)
...
@require_http_methods(["GET"])
def api_detalle_usuario(request, id_usuario: int):
    val = serial.DetalleUsuarioEntradaSerializer(data={"id_usuario": id_usuario})
    if not val.is_valid():
...
def api_guardar_usuario(request):
    ...
    val = serial.GuardarUsuarioSerializer(data=data)
    if not val.is_valid():
...
@require_http_methods(["POST"])
def api_eliminar_usuario(request, id_usuario: int):
    val = serial.EliminarUsuarioSerializer(data={"id_usuario": id_usuario})
    if not val.is_valid():
        return JsonResponse({"errors": val.errors}, status=400)
    val.aplicar()
